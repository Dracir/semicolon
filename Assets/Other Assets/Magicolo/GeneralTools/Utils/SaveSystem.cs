	...
	public class SaveSystem : MonoBehaviour {
		...
		public static string[] SerializeData(Dictionary<string, object> dataDict) {
			...
			for (int i = 0; i < keys.Count; i++) {
				data[i] = dataDict[keys[i]].GetType().AssemblyQualifiedName;
				data[i] += KeyValueSeparator;
				data[i] += keys[i];
				data[i] += KeyValueSeparator;
				data[i] += dataDict[keys[i]].SerializeXml();
				data[i] += '\n';
				data[i] += ObjectSeparator;
			}
			return data;
		}
		...
		public static Dictionary<string, object> DeserializeData(string[] data) {
			...
			for (int i = 0; i < data.Length; i++) {
				...
				dataDict[typeKeyValue[1]] = typeKeyValue[2].DeserializeXml(Type.GetType(typeKeyValue[0]));
			}
			return dataDict;
		}
	}
