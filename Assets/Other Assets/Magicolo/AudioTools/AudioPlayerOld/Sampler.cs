...
public class Sampler : MonoBehaviour {
	...
	public static AudioSource Play(string instrumentName, int midiNote, float velocity, GameObject sourceObject = null, float delay = 0, AudioPlayerOld.SyncMode syncMode = AudioPlayerOld.SyncMode.None) {
		...
		if (instrument.sendToPD) audioSource.gameObject.GetOrAddComponent<AudioGainManager>().Initialize(instrument.name + "~");
	...
	public static List<AudioSource> Play(string instrumentName, int[] midiNotes, float velocity, GameObject sourceObject = null, float delay = 0, AudioPlayerOld.SyncMode syncMode = AudioPlayerOld.SyncMode.None) {
		List<AudioSource> audioSources = new List<AudioSource>();
	...
	public static List<AudioSource> PlayRepeating(float repeatRate, string instrumentName, int midiNote, float velocity, GameObject sourceObject = null, float delay = 0, AudioPlayerOld.SyncMode syncMode = AudioPlayerOld.SyncMode.None) {
		return AudioPlayerOld.PlayRepeating(repeatRate, new Note(instrumentName, midiNote, velocity), sourceObject, delay, syncMode);
	}
	
	public static List<AudioSource> PlayRepeating(float repeatRate, string instrumentName, int[] midiNotes, float velocity, GameObject sourceObject = null, float delay = 0, AudioPlayerOld.SyncMode syncMode = AudioPlayerOld.SyncMode.None) {
		return AudioPlayerOld.PlayRepeating(repeatRate, new Note(instrumentName, midiNotes, velocity), sourceObject, delay, syncMode);
	}
	...
	public static void Stop(AudioSource audioSource, float delay = 0) {
		AudioPlayerOld.Stop(audioSource, delay);
	}
	...
	public static void StopImmediate(List<AudioSource> audioSources, float delay = 0) {
		AudioPlayerOld.StopImmediate(audioSources, delay);
	}
	
	public static void StopAllImmediate(float delay = 0) {
		AudioPlayerOld.StopAllImmediate(delay);
		Instance.coroutineHolder.RemoveAllCoroutines();
	}
...
public class Instrument {
	...
	[Min(0)] public float idleThreshold = 5;
	...
	[Min(0)] public float minNote = 48;
	[Min(0)] public float maxNote = 72;
	
	public bool velocityLayersChanged {
		get {
			bool changed = pVelocityLayers != velocityLayers;
	...
	public void InitializeClips() {
		if (audioClips != null && audioClips.Length == 16 * 128)
			return;
		audioSources = new AudioSource[16 * 128];
		...
		if (audioClips != null) {
			for (int i = 0; i < audioClips.Length; i++) {
				if (audioClips[i] != null)
					originalClips[i] = audioClips[i].name;
			}
		}
	}
	...
	void RemoveInactiveSources(List<AudioSource> toClean) {
		List<AudioSource> toRemove = new List<AudioSource>();
		foreach (AudioSource audioSource in toClean) {
			if (audioSource == null)
