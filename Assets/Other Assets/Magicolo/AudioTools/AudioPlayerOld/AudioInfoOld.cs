...
[ExecuteInEditMode]
public class AudioInfoOld : MonoBehaviour {
	...
	public void UpdateInfo() {
		...
		if (AudioPlayerOld.Instance != null) {
			if (AudioPlayerOld.Instance.buses != null) {
				if (buses == null)
					UpdateBuses();
				else if (buses.Length != AudioPlayerOld.Instance.buses.Length)
						UpdateBuses();
					else {
						for (int i = 0; i < AudioPlayerOld.Instance.buses.Length; i++) {
							if (buses[i].name != AudioPlayerOld.Instance.buses[i].name) {
								UpdateBuses();
								break;
							}
						}
			...
			if (AudioPlayerOld.Instance.rTPCs != null) {
				if (rTPCs == null)
					UpdateRTPCs();
				else if (rTPCs.Length != AudioPlayerOld.Instance.rTPCs.Length)
						UpdateRTPCs();
					else {
						for (int i = 0; i < AudioPlayerOld.Instance.rTPCs.Length; i++) {
							if (rTPCs[i].name != AudioPlayerOld.Instance.rTPCs[i].name) {
								UpdateRTPCs();
								break;
							}
						}
		...
		if (fadeOutCurve == null) {
			fadeOutCurve = new AnimationCurve(new Keyframe[2] {
				new Keyframe(0, 1),
				new Keyframe(1, 0)
			});
		}
