...
public class PDPlayerEditor : CustomEditorBase {
	...
	void ShowGeneralSettings(){
		...
		EditorGUILayout.PropertyField(serializedObject.FindProperty("maxVoices"), new GUIContent("Max Voices", "The maximum number simultaneous voices.\nReduce this if the PDPlayer takes too much CPU."));
	...
	void ShowDefaultSettings() {
		...
		EditorGUILayout.LabelField(new GUIContent("Default Module Settings", "Default settings for new modules."));
	...
	void ShowModule(){
		if (currentModuleProperty.isExpanded){
			...
			EditorGUI.BeginDisabledGroup(Application.isPlaying);
			currentModule.SetUniqueName(EditorGUILayout.TextField(currentModule.Name), "default", editorHelper.modules);
			EditorGUI.EndDisabledGroup();
