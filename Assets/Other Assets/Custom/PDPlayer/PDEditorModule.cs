...
namespace Magicolo.AudioTools {
	...
	public class PDEditorModule : Magicolo.GeneralTools.INamable {
		...
		public AudioItem.States State {
			get {
				return Application.isPlaying ? pdPlayer.moduleManager.GetModule(name).State : state;
			}
		}
		...
		public float Volume {
			...
			set {
				...
				if (Application.isPlaying) {
					pdPlayer.moduleManager.SetVolume(name, volume);
				}
			}
		}
		...
		public GameObject Source {
			...
			set {
				source = value;
				if (Application.isPlaying) {
					pdPlayer.moduleManager.GetModule(name).spatializer.Source = source;
				}
			}
		}
		...
		public PDSpatializer.RolloffMode VolumeRolloff {
			...
			set {
				volumeRolloff = value;
				if (Application.isPlaying) {
					pdPlayer.moduleManager.GetModule(name).spatializer.VolumeRolloff = volumeRolloff;
				}
			}
		}
		...
		public float MinDistance {
			...
			set {
				minDistance = value;
				if (Application.isPlaying) {
					pdPlayer.moduleManager.GetModule(name).spatializer.MinDistance = minDistance;
				}
			}
		}
		...
		public float MaxDistance {
			...
			set {
				maxDistance = value;
				if (Application.isPlaying) {
					pdPlayer.moduleManager.GetModule(name).spatializer.MaxDistance = maxDistance;
				}
			}
		}
		...
		public float PanLevel {
			...
			set {
				panLevel = value;
				if (Application.isPlaying) {
					pdPlayer.moduleManager.GetModule(name).spatializer.PanLevel = panLevel;
				}
			}
		}
