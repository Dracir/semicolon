...
public class PDPlayer : MonoBehaviour {
	...
	static public Dictionary<string, Module> Modules;
	static PDPlayer instance;
	static public PDPlayer Instance {
		get {
			if (instance == null) {
	...
	void Start() {
		if (Application.isPlaying) {
			...
			if (FindObjectsOfType<PDPlayer>().Length > 1) {
	...
	static public void Play(string moduleName, float delay = 0, AudioPlayer.SyncMode syncMode = AudioPlayer.SyncMode.None) {
		if (Modules.ContainsKey(moduleName))
			Modules[moduleName].Play(delay, syncMode);
	}
	
	static public void Stop(string moduleName, float delay = 0) {
		if (Modules.ContainsKey(moduleName))
			Modules[moduleName].Stop(delay);
	}
	
	static public void Pause(string moduleName, float delay = 0) {
		if (Modules.ContainsKey(moduleName))
			Modules[moduleName].Pause(delay);
	}
	
	static public void Resume(string moduleName, float delay = 0) {
		if (Modules.ContainsKey(moduleName))
			Modules[moduleName].Resume(delay);
	}
	
	static public float GetVolume(string moduleName) {
		float volume = 0;
		if (Modules.ContainsKey(moduleName))
			volume = Modules[moduleName].volume;
		return volume;
	}

	static public void SetVolume(string moduleName, float targetVolume) {
		if (Modules.ContainsKey(moduleName))
			Modules[moduleName].SetVolume(targetVolume);
	}
	
	static public void SetVolume(string moduleName, float targetVolume, float time) {
		if (Modules.ContainsKey(moduleName))
			Modules[moduleName].SetVolume(targetVolume, time);
	}
	
	static public bool SendValue(string sendName, object toSend) {
		int success = -1;
	...
	static public string ToPDType(System.Type type) {
		string pdType = "";
	...
	public class Module {
		...
		public string name {
			get { return Name; }
			set { Name = PDPlayer.GetUniqueName(Instance.modules, value, Name); }
		}
		...
		[Tooltip("If implemented, the module will be panned and attenuated using the source's position in relation to the listener's position.\nIf left empty, the module will not be panned or attenuated.\n\nThe source's position can be recuperated with a receiver named 'Prefix'_SourcePosition.\nAdditionnaly, the listener's angle and distance in relationship to the source can be recuperated with a receiver named 'Prefix'_ListenerAngle and 'Prefix'_ListenerDistance.")]
		...
		public void Play(float delay = 0, AudioPlayer.SyncMode syncMode = AudioPlayer.SyncMode.None) {
			delay = (float)(AudioPlayer.GetDelayToSync(syncMode) + AudioPlayer.GetAdjustedDelay(delay, syncMode));
		...
		public void SetVolume(float targetVolume) {
			...
			PDPlayer.SendValue(name + suffixes.setVolume, volume / 100);
		}
		
		public void SetVolume(float targetVolume, float time) {
			volume = Mathf.Clamp(targetVolume, 0, 100);
			PDPlayer.SendValue(name + suffixes.setVolume, new [] {
				volume / 100,
		...
		public bool SendValue(string sendSuffix, object toSend) {
			return PDPlayer.SendValue(name + sendSuffix, toSend);
		}
		...
		IEnumerator PlayAfterDelay(float delay = 0) {
			...
			isPlaying = true;
			PDPlayer.SendValue(name + suffixes.play, 1);
		}
		...
		IEnumerator StopAfterDelay(float delay = 0) {
			...
			isPlaying = false;
			PDPlayer.SendValue(name + suffixes.stop, 0);
		}
		...
		IEnumerator PauseAfterDelay(float delay = 0) {
			...
			PDPlayer.SendValue(name + suffixes.pause, 0);
		}
		...
		IEnumerator ResumeAfterDelay(float delay = 0) {
			...
			PDPlayer.SendValue(name + suffixes.resume, 1);
		}
		...
		void Spatialize() {
			if (source == null) {
				PDPlayer.SendValue(name + "_HRFLeft", 20000);
				PDPlayer.SendValue(name + "_HRFRight", 20000);
				PDPlayer.SendValue(name + "_PanLeft", 1);
				PDPlayer.SendValue(name + "_PanRight", 1);
				PDPlayer.SendValue(name + "_Attenuation", 1);
			}
			else {
				...
				PDPlayer.SendValue(name + "_HRFLeft", hrfLeft);
				PDPlayer.SendValue(name + "_HRFRight", hrfRight);
				PDPlayer.SendValue(name + "_PanLeft", panLeft);
				PDPlayer.SendValue(name + "_PanRight", panRight);
				PDPlayer.SendValue(name + "_Attenuation", attenuation);
				PDPlayer.SendValue(name + "_SourcePosition", source.transform.position);
				PDPlayer.SendValue(name + "_ListenerAngle", angle);
				PDPlayer.SendValue(name + "_ListenerDistance", distance);
			}
			changed = false;
		}
	}
	...
	public class Send {
		...
		public void UpdateObjectToSend() {
			if (members.Count > 0) {
				if (members[valueIndex].MemberType == MemberTypes.Field)
					objectToSend = ((FieldInfo)members[valueIndex]).GetValue(currentComponent);
				else if (members[valueIndex].MemberType == MemberTypes.Property)
					objectToSend = ((PropertyInfo)members[valueIndex]).GetValue(currentComponent, null);
			}
		}
