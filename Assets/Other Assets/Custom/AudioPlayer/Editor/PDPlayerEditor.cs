...
public class PDPlayerEditor : CustomEditorBase {
	...
	PDPlayer pureDataPlayer;
	PDPlayer.Module currentModule;
	
	public override void OnInspectorGUI(){
		pureDataPlayer = (PDPlayer) target;
	...
	void ShowModules(){
		if (LargeAddElementButton(serializedObject.FindProperty("modules"), "Add New Module")){
			pureDataPlayer.modules[pureDataPlayer.modules.Length - 1] = new PDPlayer.Module();
			pureDataPlayer.modules[pureDataPlayer.modules.Length - 1].name = PDPlayer.GetUniqueName(pureDataPlayer.modules, "default");
		}
	...
	void ShowSends(PDPlayer.Module module, SerializedProperty moduleProperty){
		...
		if (module.showSends){
			...
			for (int i = 0; i < module.sends.Length; i++) {
				PDPlayer.Send send = module.sends[i];
				...
				if (send.showing){
					...
					if (send.type == PDPlayer.Send.SendTypes.Audio){
	...
	void OnSendAdded(SerializedProperty addedElement){
		currentModule.sends[currentModule.sends.Length - 1] = new PDPlayer.Send();
	}
	
	void ShowComponents(PDPlayer.Send send){
	...
	void ShowFieldsAndProperties(PDPlayer.Send send){
		...
		if (send.currentComponent != null){
			...
			foreach (MemberInfo member in send.members) {
				...
				if (memberType != null) {
					displayedOptions.Add(string.Format("{0}:{1}", member.Name.Capitalize(), PDPlayer.ToPDType(memberType)));
				}
			}
		}
